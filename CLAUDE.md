...
├── config.py           # Konstanty: trhy, bookmakeři, prahy pro steam/CLV
...
│
├── scripts/
│   └── auto_fetch.py   # Smart auto-fetch pro GitHub Actions — dynamic interval per match
│
├── .github/workflows/
│   └── auto_fetch.yml  # Cron: každých 30 minut (`:00` a `:30`) → spouští auto_fetch.py
│
├── tests/
│   └── test_database.py
│
└── pages/
    ├── matches.py      # Fetch UI, preset systém, liga-grouped seznam zápasů s "Detail →"
    ├── match_detail.py # Detail zápasu: 4 taby (Přehled kurzů, Změny, Steam, CLV)
    ├── movement.py     # Pinnacle pivot kurzů v čase (pre-kickoff only)
    ├── signals.py      # Pinnacle opening→closing pohyby + steam badge pro všechny aktivní zápasy
    ...
    ├── utils.py        # Sdílené helpery: highlight_pivot, sport_label_map, to_local_str
...
- **Fetchované bookmakery:** Pinnacle + Williamhill + Nordicbet (minimum pro steam detekci = 3)
- **Zobrazované bookmakery:** pouze Pinnacle ve všech přehledových tabulkách; Williamhill/Nordicbet slouží výhradně pro detekci steam moves
...
`scripts/auto_fetch.py` běží přes GitHub Actions každých 30 minut (cron `:00` a `:30`).
Fetchuje jen presety kde `auto_fetch=1`. Dynamic interval podle vzdálenosti výkopu:
...
Script si sám hlídá `last_used_at` presetu — pokud interval ještě neuplynul, fetch přeskočí.
Credit floor: 30 kreditů (konfigurovatelné v `user_config.auto_fetch_credit_floor`).
...
- `get_clv_df`, `get_results_df`, `get_presets`, `get_movement_overview`
...
- Persistuje: sport_key, competition, markets (JSON list), bookmakers (JSON list), regions, auto_fetch (bool)
- Poslední výběr se ukládá do `user_config` (jen při změně hodnoty)

## Navigace a UX

- `pages/matches.py` zobrazuje zápasy seskupené per liga (`st.expander`) s tlačítkem "Detail →"
- Klik na "Detail →" nastaví `st.session_state.selected_match_id` + `selected_match_label` a přejde na `match_detail.py`
- `pages/match_detail.py` čte `selected_match_id` ze session_state; pokud chybí, zobrazí warning a stop
- Časy se zobrazují v **Prague timezone** (Europe/Prague) ve formátu `DD.MM HH:MM` — helper `to_local_str()` v `pages/utils.py`
- Filtry defaultují na **Pinnacle** všude kde je výběr bookmakera

## Definice Opening / Closing

Kritická konzistence — používat vždy tuto definici:

- **Opening** = první snapshot per bookmaker (`MIN(snapshot_time) WHERE bookmaker=o.bookmaker`)
- **Closing** = poslední snapshot per bookmaker PŘED výkopem (`MAX(snapshot_time) WHERE bookmaker=o.bookmaker AND snapshot_time <= commence_time`)
- **Pivot tabulka** zobrazuje pouze pre-kickoff snapshoty → poslední řádek = Closing
- Post-kickoff snapshoty jsou live kurzy — nejsou relevantní pro CLV analýzu, zobrazí se jen jejich počet v caption

Funkce v `db/queries.py`:
- `get_opening_odds(match_id, market)` — per-bookmaker MIN
- `get_closing_odds(match_id, market)` — per-bookmaker MAX před kickoff
- `get_movement_overview()` — Pinnacle opening+closing pro všechny aktivní zápasy; `last_snap` filtrován na `snapshot_time <= commence_time`
...
- Formátování čísel: kurzy `{:.2f}`, delty `{:+.2f}`, CLV abs `{:+.3f}`, CLV % `{:+.2f}`
...
- `pages/utils.py:to_local_str(series)` — konvertuje UTC ISO string series na Prague timezone string `DD.MM HH:MM`
